      ...
      class << self
        ...
        def get_articles_24hdz(url_media_array, media)
          ...
          articles_url_24hdz.map do |link|
            articles_url_24hdz_after_check << link unless Article.where(medium_id: media.id,
          ...
          articles_url_24hdz_after_check.map do |link|
            ...
            new_article.date_published = article.at('time[datetime]')['datetime'].to_datetime.change({ hour: 0,
                                                                                                       min: 0, sec: 0 }) + (1.0 / 24)
